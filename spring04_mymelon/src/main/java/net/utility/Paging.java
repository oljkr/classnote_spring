...
public class Paging {  
  ...
  public String paging1(int totalRecord, int nowPage, int recordPerPage, String col, String word, String filenm){
    ...
    int nowGrp = (int)(Math.ceil((double)nowPage/pagePerBlock));    // 현재 그룹. 현재 페이지가 12라면 2=.ceil((12/10)(=1.2))
    int startPage = ((nowGrp - 1) * pagePerBlock) + 1; // 특정 그룹의 페이지 목록 시작 11 = (2-1)*10+1
    int endPage = (nowGrp * pagePerBlock);             // 특정 그룹의 페이지 목록 종료  20 = 2*10
    ...
    int _nowPage = (nowGrp-1) * pagePerBlock; // 10개 이전 페이지로 이동. 10 = (2-1)*10. 이전 단계의 끝페이지를 _nowPage에 대입.
    if (nowGrp >= 2){
    ...
    _nowPage = (nowGrp * pagePerBlock)+1; // 10개 다음 페이지로 이동 21 = (2*10)+1. 이후 단계의 첫페이지를 _nowPage에 대입
    if (nowGrp < totalGrp){
